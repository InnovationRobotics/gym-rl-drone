...
class SAC(OffPolicyRLModel):
    ...
    def learn(self, total_timesteps, callback=None,
              log_interval=4, tb_log_name="SAC", reset_num_timesteps=True, replay_wrapper=None, eval_freq=50000):
            ...
            for step in range(total_timesteps):
                if callback is not None:
                    ...
                    if callback(locals(), globals()) is False:
                        break
                if (step+1) % eval_freq == 0:   ### model evaluation
                    eval_tries = 4
                    for _ in range(eval_tries):
                        while True:
                            ...
                            self.env.render()
                            obs, reward, done, info = self.env.step(unscaled_action)
                            # print('action: ', action, 'pos: ', obs[0:3], 'vel: ', obs[3:6], 'rew: ', reward)
                            if done:
                                self.env.reset()
                                break
                ...
                # self.env.render()
                # print('state: ', obs, 'action: ', unscaled_action)
